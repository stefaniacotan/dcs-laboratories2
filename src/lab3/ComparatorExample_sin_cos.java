...
public class ComparatorExample_sin_cos {
...
// FLRS table for T1 that makes the selection according to the result P0-P1 (positive or negative)
    ...
    public ComparatorExample_sin_cos() {
...
// add the arcs and the weights corresponding to the Petri Net petriNet.addArcFromPlaceToTransition(p0Inp, t0, 1.0);
...
// placing the fuzzyficated token
            ...
            inps.put(p0Inp, driver.fuzzifie(Math.sin(i*2*Math.PI / 500 * 10)));
            inps.put(p1Inp, driver.fuzzifie(Math.cos(i*2*Math.PI / 500 * 10)));
